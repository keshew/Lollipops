...
class LollipopsGameData: ObservableObject {
...
class LollipopsGameSpriteKit: SKScene, SKPhysicsContactDelegate {
    var game: LollipopsGameData?
    let rows = 6
    let cols = 6
...
//                                  ImageName.item5.rawValue,
    ...
    func updateTimer() {
        if game?.timeLeft ?? 0 > 0 {
            ...
            if LollipopsUserDefaultsManager.defaults.object(forKey: Keys.countOfLifes.rawValue) as? Int ?? 0 > 1 {
                game?.isLose = true
            } else {
    ...
    func setupView() {
        ...
        lives.attributedText = NSAttributedString(string: "\(LollipopsUserDefaultsManager.defaults.object(forKey: Keys.countOfLifes.rawValue) as? Int ?? 0)", attributes: [
            NSAttributedString.Key.font: UIFont(name: "SpicyRice-Regular", size: 34)!,
            NSAttributedString.Key.foregroundColor: UIColor.yellow,
            NSAttributedString.Key.strokeColor: UIColor.red,
            NSAttributedString.Key.strokeWidth: -3
        ...
        coinCount.attributedText = NSAttributedString(string: "\(LollipopsUserDefaultsManager.defaults.object(forKey: Keys.countOfMoney.rawValue) as? Int ?? 0)", attributes: [
            NSAttributedString.Key.font: UIFont(name: "SpicyRice-Regular", size: 34)!,
            NSAttributedString.Key.foregroundColor: UIColor.yellow,
            NSAttributedString.Key.strokeColor: UIColor.red,
            NSAttributedString.Key.strokeWidth: -3
...
struct LollipopsGameView: View {
    @StateObject var gameModel =  LollipopsGameViewModel()
    @StateObject private var gameData = LollipopsGameData()
...
#Preview {
    ...
    return LollipopsGameView(navigationPath: .constant(navigationPath))
}
